$(document).ready(function () {
    ...
    // On ready document get all beers on the page
    $.ajax({
        type: "GET",
        url: beerUrl,
        success: getBeers,
    ...
    // and append it to div container in allBeers.html
    function getBeers(beers) {
        ...
        for (var i = 0; i < allBeers.length; i++) {
            ...
            if (beerPic === null || beerPic === "") {
                ...
                console.log("IF DEFAULT PIC" + beerPic )
    ...
    let abvBtnClicked = false;
    $("#abvBtn").click(function () {
        $("#container").empty();

        if (abvBtnClicked === false) {
            $.ajax({
                type: "GET",
                url: beerUrl + "/sort?ABVCriteria=desc",
                success: getBeers,
                error: console.log("Error in sorting beers by ABV")
            });
            $("#abvBtn").children().removeClass().addClass('fa fa-sort-desc');
            abvBtnClicked = true;
        } else {
            $.ajax({
                type: "GET",
                url: beerUrl + "/sort?ABVCriteria=asc",
                success: getBeers,
                error: console.log("Error in sorting beers by ABV")
            });
            $("#abvBtn").children().removeClass().addClass('fa fa-sort-asc');
            abvBtnClicked = false;
        }
    });

    let rateBtnClicked = false;
    $("#rateBtn").click(function () {
        $("#container").empty();

        if (rateBtnClicked === false) {
            $.ajax({
                type: "GET",
                url: beerUrl + "/sort?ratingCriteria=desc",
                success: getBeers,
                error: console.log("Error in sorting beers by rating")
            });
            $("#rateBtn").children().removeClass().addClass('fa fa-sort-desc');
            rateBtnClicked = true;
        } else {
            $.ajax({
                type: "GET",
                url: beerUrl + "/sort?ratingCriteria=asc",
                success: getBeers,
                error: console.log("Error in sorting beers by ABV")
            });
            $("#rateBtn").children().removeClass().addClass('fa fa-sort-asc');
            rateBtnClicked = false;
        }
    });

    let alphaBtnClicked = false;
    $("#alphaBtn").click(function () {
        $("#container").empty();

        if (alphaBtnClicked === false) {
            $.ajax({
                type: "GET",
                url: beerUrl + "/sort?nameCriteria=desc",
                success: getBeers,
                error: console.log("Error in sorting beers by name")
            });
            $("#alphaBtn").children().removeClass().addClass('fa fa-sort-desc');
            alphaBtnClicked = true;
        } else {
            $.ajax({
                type: "GET",
                url: beerUrl + "/sort?nameCriteria=asc",
                success: getBeers,
                error: console.log("Error in sorting beers by beer name")
            });
            $("#alphaBtn").children().removeClass().addClass('fa fa-sort-asc');
            alphaBtnClicked = false;
        }
    });

    //Jquery for filter by country
    ...
    countryFilter.addEventListener('change', function () {
        ...
        $.ajax({
            type: "GET",
            url: beerUrl + `/filter?country=${country}`,
            success: getBeers,
            error: console.log("Error in filter beers by country")
        });

    }, false);

    //Jquery for filter by style
    ...
    styleFilter.addEventListener('change', function () {
        ...
        $.ajax({
            type: "GET",
            url: beerUrl + `/filter?style=${style}`,
            success: getBeers,
            error: console.log("Error in filter beers by style")
        });
    ...
    $("body").on("click", "#wishBtn", function () {
        ...
        console.log(beerId);
        console.log(userUrl + `/${username}/markAsWish?beerId=${beerId}`);

        $.ajax({
            type: "PUT",
            url: userUrl + `/${username}/markAsWish?beerId=${beerId}`,
            success: () => {
                swal("Done!", "Yay, you have added this beer in your wishlist!", "success");
            },
            error: errorFunction
        });
    ...
    $("body").on("click", "#drankBtn", function () {
        let beerId = $(this).attr('value');
        console.log(beerId);
        console.log(userUrl + `/${username}/markAsDrank?beerId=${beerId}`);

        $.ajax({
            type: "PUT",
            url: userUrl + `/${username}/markAsDrank?beerId=${beerId}`,
            success: () => {
...
// ==============================================//
    ...
    //SHOULD ATTACH LINK TO CARD BODY FOR EVENT LISTENER TO ATTACH
    ...
    function getBeerModalSuccess(res) {
        ...
        let beerObj = ` <div class="card mb-3" id="beer-card">
            ...
            <img class="card-img-top" src="data:image/jpeg;charset=utf-8;base64,` + beerPic + `"  width="200px"/>
             ...
             <button  id="remove-drankBtn" type="button" class="btn btn-outline-dark" 
              title="Remove from tasted beers" value="${beer.beerId}">
              <span class="tooltiptext">Remove from tasted</span>
    ...
    function getTagsSuccess(res) {
        ...
        for (let i = 0; i < tagsArrSize; i++) {
            ...
            let tag = `
            <li >${currTag.tagBody}<span value="${currTag.tagId}" beerId="" class="close">x</span></li>
            `
            $("#tag-body").append(tag);
        }
    }
